...
class TestDataSeeder extends Seeder
{
    ...
    private function seedRequesters(int $count): void
    {
        User::factory()
            ->requester()
            ->count($count)
    ...
    private function seedProjectPortfolio(Collection $staffMembers, Collection $requesters): void
    {
        $progression = array_values(ProjectStatus::getProgressStages());
        $statuses = collect();

        foreach ($this->projectStatusDistribution() as [$status, $count]) {
            for ($i = 0; $i < $count; $i++) {
                $statuses->push($status);
            }
        }

        foreach ($statuses->shuffle() as $status) {
            $timeline = $this->projectTimeline($status);

            $project = Project::factory()
                ->for($requesters->random(), 'user')
                ->create([
                    'status' => $status,
    ...
    /**
     * @return array<int, array{0: ProjectStatus, 1: int}>
     */
    private function projectStatusDistribution(): array
    {
        return [
            [ProjectStatus::IDEATION, 15],
            [ProjectStatus::FEASIBILITY, 10],
            [ProjectStatus::SCOPING, 8],
            [ProjectStatus::SCHEDULING, 8],
            [ProjectStatus::DETAILED_DESIGN, 6],
            [ProjectStatus::DEVELOPMENT, 6],
            [ProjectStatus::TESTING, 5],
            [ProjectStatus::DEPLOYED, 3],
    ...
    {
        $faker = fake();
        $hasReached = $this->projectHasReachedStage($project, $status);
        $staffId = $hasReached ? $this->pickStaffId($staffMembers, [$project->user_id]) : null;
        ...
        $factory = match ($status) {
            ...
            ProjectStatus::FEASIBILITY => function () use ($faker, $staffId) {
                return [
                    'assessed_by' => $staffId,
            ...
            ProjectStatus::SCOPING => function () use ($faker, $staffId) {
                $skillIds = $this->sampleSkillIds();
            ...
            ProjectStatus::SCHEDULING => function () use ($faker, $staffMembers, $project, $hasReached) {
                $assigned = $hasReached ? $this->pickStaffId($staffMembers, [$project->user_id]) : null;
                // Spread projects across 12 months for better roadmap visualization
                $startOffset = random_int(-60, 180); // Start: 2 months ago to 6 months ahead
                $duration = random_int(30, 120); // Duration: 1-4 months
                ...
                $team = $hasReached
                    ? $this->randomStaffIds($staffMembers, random_int(2, 4), [$assigned, $project->user_id])
                    : [];

                return [
                    'assigned_to' => $assigned,
                    'key_skills' => $hasReached ? $this->skillNamesFor($existingSkills) : null,
                    'cose_it_staff' => $team,
            ...
            ProjectStatus::DEVELOPMENT => function () use ($faker, $staffMembers, $project, $hasReached, $staffId) {
                $lead = $staffId;
                $team = $hasReached
                    ? collect($this->randomStaffIds($staffMembers, random_int(1, 3), [$lead, $project->user_id]))
                        ->prepend($lead)
                        ->unique()
                        ->values()
                        ->all()
            ...
            ProjectStatus::TESTING => function () use ($faker, $staffId) {
                return [
                    'test_lead' => $staffId,
                    'service_function' => $faker->sentence(),
            ...
            ProjectStatus::DEPLOYED => function () use ($faker, $staffId) {
                return [
                    'deployment_lead_id' => $staffId,
    ...
    private function projectHasReachedStage(Project $project, ProjectStatus $stage): bool
    {
        $progression = array_values(ProjectStatus::getProgressStages());
        $projectIndex = array_search($project->status, $progression, true);
        $stageIndex = array_search($stage, $progression, true);

        if ($projectIndex === false || $stageIndex === false) {
            return false;
        }

        return $projectIndex >= $stageIndex;
    }
    ...
    private function updateBusynessFromWorkload(): void
    {
        ...
        foreach ($staffMembers as $member) {
            $activeCount = $member->activeAssignedProjectCount();
    ...
    {
        return [
            ['forenames' => 'Steve', 'surname' => 'Arnott', 'week_1' => Busyness::MEDIUM, 'week_2' => Busyness::HIGH],
            ['forenames' => 'Humphrey', 'surname' => 'Goodman', 'week_1' => Busyness::LOW, 'week_2' => Busyness::MEDIUM],
            ['forenames' => 'Ted', 'surname' => 'Hastings', 'week_1' => Busyness::HIGH, 'week_2' => Busyness::HIGH],
            ['forenames' => 'Neville', 'surname' => 'Parker', 'week_1' => Busyness::MEDIUM, 'week_2' => Busyness::MEDIUM],
            ['forenames' => 'Christopher', 'surname' => 'Foyle', 'week_1' => Busyness::LOW, 'week_2' => Busyness::LOW],
            ['forenames' => 'Robbie', 'surname' => 'Lewis', 'week_1' => Busyness::MEDIUM, 'week_2' => Busyness::HIGH],
            ['forenames' => 'Richard', 'surname' => 'Poole', 'week_1' => Busyness::HIGH, 'week_2' => Busyness::MEDIUM],
            ['forenames' => 'Kate', 'surname' => 'Fleming', 'week_1' => Busyness::LOW, 'week_2' => Busyness::MEDIUM],
            ['forenames' => 'Ellie', 'surname' => 'Miller', 'week_1' => Busyness::MEDIUM, 'week_2' => Busyness::LOW],
            ['forenames' => 'Jackson', 'surname' => 'Brodie', 'week_1' => Busyness::HIGH, 'week_2' => Busyness::HIGH],
            ['forenames' => 'Cormoran', 'surname' => 'Strike', 'week_1' => Busyness::MEDIUM, 'week_2' => Busyness::HIGH],
            ['forenames' => 'Marcella', 'surname' => 'Backland', 'week_1' => Busyness::HIGH, 'week_2' => Busyness::MEDIUM],
            ['forenames' => 'Jack', 'surname' => 'Frost', 'week_1' => Busyness::UNKNOWN, 'week_2' => Busyness::LOW],
            ['forenames' => 'John', 'surname' => 'Watson', 'week_1' => Busyness::LOW, 'week_2' => Busyness::MEDIUM],
            ['forenames' => 'Jim', 'surname' => 'Taggart', 'week_1' => Busyness::HIGH, 'week_2' => Busyness::HIGH],
            ['forenames' => 'Tom', 'surname' => 'Barnaby', 'week_1' => Busyness::MEDIUM, 'week_2' => Busyness::MEDIUM],
            ['forenames' => 'Endeavour', 'surname' => 'Morse', 'week_1' => Busyness::MEDIUM, 'week_2' => Busyness::HIGH],
            ['forenames' => 'Catherine', 'surname' => 'Cawood', 'week_1' => Busyness::HIGH, 'week_2' => Busyness::HIGH],
            ['forenames' => 'Cassie', 'surname' => 'Stuart', 'week_1' => Busyness::LOW, 'week_2' => Busyness::LOW],
            ['forenames' => 'Jane', 'surname' => 'Marple', 'week_1' => Busyness::LOW, 'week_2' => Busyness::MEDIUM],
            ['forenames' => 'Vera', 'surname' => 'Stanhope', 'week_1' => Busyness::HIGH, 'week_2' => Busyness::MEDIUM],
            ['forenames' => 'Frank', 'surname' => 'Hathaway', 'week_1' => Busyness::MEDIUM, 'week_2' => Busyness::LOW],
            ['forenames' => 'Alec', 'surname' => 'Hardy', 'week_1' => Busyness::HIGH, 'week_2' => Busyness::HIGH],
            ['forenames' => 'James', 'surname' => 'Hathaway', 'week_1' => Busyness::MEDIUM, 'week_2' => Busyness::HIGH],
            ['forenames' => 'John', 'surname' => 'Luther', 'week_1' => Busyness::HIGH, 'week_2' => Busyness::HIGH],
            ['forenames' => 'John', 'surname' => 'Barnaby', 'week_1' => Busyness::MEDIUM, 'week_2' => Busyness::MEDIUM],
            ['forenames' => 'Jack', 'surname' => 'Mooney', 'week_1' => Busyness::LOW, 'week_2' => Busyness::MEDIUM],
            ['forenames' => 'Sunny', 'surname' => 'Khan', 'week_1' => Busyness::MEDIUM, 'week_2' => Busyness::LOW],
            ['forenames' => 'Sherlock', 'surname' => 'Holmes', 'week_1' => Busyness::HIGH, 'week_2' => Busyness::HIGH],
            ['forenames' => 'Jonathan', 'surname' => 'Creek', 'week_1' => Busyness::LOW, 'week_2' => Busyness::LOW],
        ];
    }
